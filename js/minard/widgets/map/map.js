    ...
    return declare([_WidgetBase, _TemplatedMixin, Evented], {
        ...
        _createMap: function(countries) {            
            queue().defer(d3.json, "geojson/world.geojson").defer(d3.csv, "csv/cities.csv").await(function (error, file1, file2) {
                ...
                d3.select("svg").selectAll("path").data(file1.features)
                    .enter().append("path").attr("d", geoPath).attr("class", "countries");
                /*    
                d3.select("svg").selectAll("circle").data(file2).enter().append("circle").style("fill", "red").attr("class", "cities").attr("r", 3)
                    .attr("cx", function (d) {
                ...
                */        
                ...
                var lines1 = d3.select("svg").selectAll("line").data(napoleon.army);
                ...
                var temp = [
                    ...
                    {lon1: 29.2, lat1: 54.3, lon2: 29.2, lat2: 54.12, date: "24 Nov 1812"},
                ...
                var rr = temps.append("g").attr("transform", "translate(0,0)");
                rr.selectAll("line").data(temp).enter().append("line")
                    .attr("x1", function(d) {
                        return projection([d.lon1, d.lat1])[0];
                    })
                    .attr("y1", function(d) {
                ...
                //rr.append("text").text(function(d) {
                //    return d.date;
                //}).style("fill", "black").attr("transform", "translate(5,5)");
                  
                var tempLine = temps.append("g").attr("transform", "translate(0," + projection([37.75,54.55])[1] + ")");
                
               
                
                 
                tempLine.selectAll("line").data(napoleon.temp).enter().append("line")
                    .attr("x1", function(d) {
                ...
                /*
                var tempScale = d3.select("svg").selectAll("line").data(temp);
                tempScale.enter().append("line")
                    .attr("x1", function(d) {
                        return projection([d.lon1, d.lat1])[0];
                    })
                    .attr("y1", function(d) {
