...
//napoleon.army.map(function(d) {console.log( d.size,d.group)})
    ...
    longs = d3.extent(napoleon.army.map(function (d) {
        return d.x2
    }))
...
//Define a scale that returns right to left or left to right in time depending on the directionality. Well, not a scale, strictly, since it can't be inverted and so on: but a function that sits on top of scales.
    ...
    lines.enter().append("line")
            ...
            .duration(function (d) {
                return Math.abs(timeScale(d, 'x2') - timeScale(d, 'x1'))
            })
            .delay(function (d) {
                return d3.min([timeScale(d, 'x1'), timeScale(d, 'x2')])
            })
            .attr("y2", function (d) {
                return d.y2
            })
    ...
    circles = cities.append("circle").attr('r', 3).style("fill", "black").style("opacity", 0).transition().delay(function (d) {
...
// to produce the nicest effect.
    ...
    tmpLayout = d3.layout.trail()
            .positioner(function (d) {
                return [proj([d.lon, 55])[0], tempScale(d.temp)]
            })
